...
class PhysicsNamespace(Namespace):
    ...
    def __init__(self, namespace=None):
        ...
        self.active_simulations: Dict[
            int, PhysicsEngine
        ] = {}  # universe_id -> PhysicsEngine
        self.simulation_clients: Dict[
            int, Set[str]
        ] = {}  # universe_id -> set of client IDs
    ...
    def handle_error(self, client_id: str, error: Exception):
        ...
        self.emit(
            "error",
            {"message": str(error), "error_count": self.error_counts[client_id]},
            room=client_id,
        )
    ...
    def leave_simulation(self, universe_id: int):
        ...
        try:
            ...
            if (
                universe_id in self.simulation_clients
                and client_id in self.simulation_clients[universe_id]
            ):
    ...
    def on_add_particle(self, data):
        ...
        try:
            ...
            self.emit(
                "simulation_state",
                {"state": state, "universe_id": universe_id},
                room=room,
            )
                ...
                with self.app.app_context():
                    for universe_id, engine in list(self.active_simulations.items()):
                        if self.simulation_clients.get(universe_id):
                            ...
                            logger.debug(
                                f"Broadcasting state for universe {universe_id} to {len(self.simulation_clients[universe_id])} clients"
                            )
