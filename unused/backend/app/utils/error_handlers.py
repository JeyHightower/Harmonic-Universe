...
def handle_websocket_error(error, context=None):
    ...
    if context:
        current_app.logger.error(
            f"WebSocket error in {context}: {error_type} - {error_message}"
        )
    else:
...
def handle_db_error(error, context=None):
    ...
    if context:
        current_app.logger.error(
            f"Database error in {context}: {error_type} - {error_message}"
        )
    else:
...
def handle_validation_error(error, context=None):
    ...
    if context:
        current_app.logger.error(
            f"Validation error in {context}: {error_type} - {error_message}"
        )
    else:
...
def handle_auth_error(error, context=None):
    ...
    if context:
        current_app.logger.error(
            f"Authentication error in {context}: {error_type} - {error_message}"
        )
    else:
        current_app.logger.error(
            f"Authentication error: {error_type} - {error_message}"
        )
...
def handle_rate_limit_error(error, context=None):
    ...
    if context:
        current_app.logger.error(
            f"Rate limit error in {context}: {error_type} - {error_message}"
        )
    else:
...
def register_error_handlers(app):
    ...
    def handle_invalid_header_error(e):
        return jsonify({"status": "error", "message": "Invalid token header"}), 401

    @app.errorhandler(NoAuthorizationError)
    def handle_no_auth_error(e):
        return (
            jsonify({"status": "error", "message": "No authorization token provided"}),
            401,
        )

    @app.errorhandler(WrongTokenError)
    def handle_wrong_token_error(e):
        return jsonify({"status": "error", "message": "Wrong token type"}), 401
    ...
    def handle_user_lookup_error(e):
        return jsonify({"status": "error", "message": "Error loading user"}), 401
