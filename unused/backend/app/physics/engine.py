...
class Particle:
    ...
    def apply_force(self, force: Vector2D):
        ...
        self.velocity = Vector2D(
            self.velocity.x + force.x / self.mass, self.velocity.y + force.y / self.mass
        )
    ...
    def constrain_to_boundary(self, boundary: Boundary):
        ...
        if boundary.type == BoundaryType.WRAP:
            ...
            self.position.x = (self.position.x - boundary.x_min) % (
                boundary.x_max - boundary.x_min
            ) + boundary.x_min
            self.position.y = (self.position.y - boundary.y_min) % (
...
class PhysicsEngine:
    ...
    def add_particle(
        self, position: Dict, velocity: Dict, mass: float = 1.0, radius: float = 0.5
    ) -> Particle:
    ...
    def constrain_to_boundary(self, particle: Particle):
        ...
        if self.boundary.type == BoundaryType.WRAP:
            ...
            if particle.position.x - particle.radius < self.boundary.x_min:
                ...
                particle.velocity.x = (
                    abs(particle.velocity.x) * self.boundary.elasticity
                )
            ...
            if particle.position.y - particle.radius < self.boundary.y_min:
                ...
                particle.velocity.y = (
                    abs(particle.velocity.y) * self.boundary.elasticity
                )
            ...
            if (
                particle.position.x - particle.radius < self.boundary.x_min
                or particle.position.x + particle.radius > self.boundary.x_max
                or particle.position.y - particle.radius < self.boundary.y_min
                or particle.position.y + particle.radius > self.boundary.y_max
            ):
    ...
    def get_performance_metrics(self) -> Dict:
        ...
        return {
            ...
            "collision_count": sum(self.collision_counts) / len(self.collision_counts)
