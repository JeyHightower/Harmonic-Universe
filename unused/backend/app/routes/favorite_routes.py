...
def add_favorite(universe_id):
    ...
    try:
        ...
        if existing_favorite:
            return (
                jsonify({"status": "error", "message": "Universe already favorited"}),
                409,
            )

        favorite = Favorite(universe_id=universe_id, user_id=current_user_id)
        db.session.add(favorite)
        db.session.commit()

        return (
            jsonify({"status": "success", "data": {"favorite": favorite.to_dict()}}),
            201,
        )
...
def remove_favorite(universe_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"status": "success", "message": "Universe removed from favorites"}
            ),
            200,
        )
    except Exception as e:
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "status": "success",
                    "data": {"favorites": [f.to_dict() for f in favorites]},
                }
            ),
            200,
        )
