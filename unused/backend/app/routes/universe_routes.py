...
from app.schemas.universe import UniverseSchema, universes_schema
...
def get_universes():
    ...
    try:
        # Check cache first
        cache_key = f'user_universes_{current_user.id}'
        universes = cache.get(cache_key)

        if not universes:
            universes = Universe.query.filter_by(user_id=current_user.id).all()
            cache.set(cache_key, universes, timeout=300)  # Cache for 5 minutes
...
def create_universe():
    ...
    try:
        ...
        # Create universe with default parameters
        universe = Universe(
            name=data['name'],
        ...
        # Clear user's universe cache
        cache.delete(f'user_universes_{current_user.id}')
...
@universe_bp.route("/api/universes/<int:id>", methods=["GET"])
@login_required
def get_universe(id):
    ...
    try:
        ...
        current_app.logger.error(f"Error fetching universe: {str(e)}")
        return jsonify({'error': 'Failed to fetch universe'}), 500


@universe_bp.route("/api/universes/<int:id>", methods=["PUT"])
@login_required
def update_universe(id):
    """Update a universe's basic information."""
    try:
        universe = Universe.query.get_or_404(id)
...
@universe_bp.route("/api/universes/<int:id>/parameters/<string:type>", methods=["PUT"])
@login_required
def update_parameters(id, type):
    """Update universe parameters of a specific type."""
    try:
        universe = Universe.query.get_or_404(id)
        ...
        # Validate parameters
        if not validate_parameters(type, data):
        ...
        current_app.websocket_manager.broadcast_parameter_update(id, type, data)
...
@universe_bp.route("/api/universes/<int:id>", methods=["DELETE"])
@login_required
...
def add_collaborator(universe_id):
    ...
    try:
        collaborator = universe_service.add_collaborator(
            universe_id, current_user_id, data
        )
...
@universe_bp.route(
    "/api/universes/<int:universe_id>/comments/<int:comment_id>", methods=["DELETE"]
)
@jwt_required()
...
def update_physics(universe_id):
    ...
    try:
        ...
        if validation_errors:
            return (
                jsonify({"error": "Invalid parameters", "details": validation_errors}),
                400,
            )
...
def update_parameters(universe_id):
    ...
    try:
        ...
        if validation_errors:
            return (
                jsonify({"error": "Invalid parameters", "errors": validation_errors}),
                400,
            )
...
def generate_music(universe_id):
    ...
    try:
        ...
        music_data = music_generator.generate(
            universe.music_parameters, duration=duration, start_time=start_time
        )
...
def render_visualization(universe_id):
    ...
    try:
        ...
        # Broadcast via WebSocket
        current_app.websocket_manager.broadcast_visualization_update(
            universe_id, render_data
        )
...
def start_collaboration(universe_id):
    ...
    try:
        ...
        room_token = current_app.websocket_manager.create_collaboration_room(
            universe_id, current_user_id, max_participants=max_participants, mode=mode
        )
