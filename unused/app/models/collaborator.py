...
class UniverseCollaborator(db.Model):
    """Model for universe collaborators."""

    __tablename__ = "universe_collaborators"
    ...
    # Relationships
    universe = db.relationship("Universe", back_populates="collaborators")
    ...
    def get_user_collaborations(user_id):
        return UniverseCollaborator.query.filter_by(user_id=user_id).all()

    @staticmethod
    def get_universe_collaborators(universe_id):
        return UniverseCollaborator.query.filter_by(universe_id=universe_id).all()
