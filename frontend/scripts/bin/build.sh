...
PROJECT_ROOT="$(dirname "$FRONTEND_ROOT")"
...
case "$PM" in
    pnpm)
        execute_command "$PATCH_ROLLUP pnpm exec node $SCRIPTS_ROOT/core/build.mjs" "Building frontend with pnpm"
        ;;
    yarn)
        execute_command "$PATCH_ROLLUP yarn node $SCRIPTS_ROOT/core/build.mjs" "Building frontend with yarn"
        ;;
    npm)
        execute_command "$PATCH_ROLLUP npx node $SCRIPTS_ROOT/core/build.mjs" "Building frontend with npm"
        ;;
    *)
