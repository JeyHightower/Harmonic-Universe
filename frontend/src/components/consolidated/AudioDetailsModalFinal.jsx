...
const AudioDetailsModalFinal = ({
  ...
  useEffect(() => {
    const fetchAudioData = async () => {
      ...
      setError(null);
      try {
        // Use our new endpoint directly
        const response = await fetch(`/api/universes/${universeId}/music/${audioId}`);
        
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.error || 'Failed to fetch audio data');
        }
  ...
  const handleDelete = async () => {
    ...
    setError(null);
    try {
      // Use our new delete endpoint directly
      const response = await fetch(`/api/universes/${universeId}/music/${audioId}`, {
        method: 'DELETE'
      });
...
AudioDetailsModalFinal.propTypes = {
...
AudioDetailsModalFinal.defaultProps = {
  sceneId: null,
  onClose: () => {},
  modalProps: {},
  isGlobalModal: false,
...
export default AudioDetailsModalFinal; 