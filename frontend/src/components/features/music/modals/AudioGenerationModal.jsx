...
import { generateAudio, saveAudio, getAudio } from "../../../services/api/audioApi";
...
const AudioGenerationModal = ({
  ...
  useEffect(() => {
    const fetchAudioData = async () => {
      if (initialData?.id) {
        try {
          setLoading(true);
          const response = await getAudio(universeId, sceneId, initialData.id);
          if (response.success) {
            setFormData({
              ...formData,
              ...response.data,
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await generateAudio(universeId, sceneId, {
