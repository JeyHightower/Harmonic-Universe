...
import { getAudio, deleteAudio } from "../../../services/api/audioApi";
...
const AudioDetailsModal = ({
  ...
  useEffect(() => {
    const fetchAudio = async () => {
      try {
        setLoading(true);
        const response = await getAudio(universeId, sceneId, audioId);
        if (response.success) {
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this audio?")) {
      try {
        const response = await deleteAudio(universeId, sceneId, audioId);
        if (response.success) {
          if (isGlobalModal) {
