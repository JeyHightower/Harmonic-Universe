...
import React, { forwardRef, useCallback, useEffect, useMemo, useState, useImperativeHandle, memo } from 'react';
 ...
 */
const SceneModal = forwardRef((props, ref) => {
  const {
  ...
  // Optional: Expose imperative methods via ref
  useImperativeHandle(ref, () => ({
    openModal: () => {
      console.log('SceneModal - Imperative open called');
    },
    closeModal: () => onClose?.(),
    focus: () => {
      // Focus the modal if needed
    },
  }));
  ...
  // useEffect to fetch scene data for edit/view/delete modes
  // Dependency array is already correct here.
  ...
  const handleSubmit = useCallback(
    ...
    [actualMode, formattedSceneId, universeId, dispatch, onClose, onSuccess, loading]
  );

  const handleDelete = useCallback(async () => {
    if (loading) {
      console.log('SceneModal - Deletion already in progress, ignoring duplicate delete');
      return;
    }
  ...
  }, [formattedSceneId, universeId, dispatch, onClose, onSuccess, loading]);

  const loadSceneData = useCallback(async () => {
    if (loading) {
      console.log('SceneModal - Data load already in progress, ignoring duplicate load');
      return;
    }
  ...
  }, [formattedSceneId, initialData, universeId, dispatch, loading]);
  ...
  const renderContent = useCallback(() => {
    // Content renders immediately (no isContentMounted check)
    ...
    const viewerData = scene || initialData;  // Compute inline to avoid deps issues
      ...
      case 'edit':
        ...
        return (
          <SceneForm
            ...
            initialData={actualMode === 'edit' ? viewerData : undefined}
    ...
    onClose,
    handleDelete,
  ]);  // Removed 'viewerData' from deps (now computed inline)
...
SceneModal.propTypes = {
  open: PropTypes.bool.isRequired, // <-- This is the required fix for 'open'

  // Include all other props used in the component
