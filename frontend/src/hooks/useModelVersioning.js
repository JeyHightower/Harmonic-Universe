...
export interface ModelVersioning {
  ...
  createVersion: (data: Partial) => Promise;
  updateVersion: (id: number, data: Partial) => Promise;
  deleteVersion: (id: number) => Promise;
  loading: boolean;
  error: string | null;
}
...
export const useModelVersioning = (modelId: number | null): ModelVersioning => {
  ...
  const createVersion = useCallback(async (data: Partial) => {
    try {
      setLoading(true);
      setError(null);
  ...
  const updateVersion = useCallback(async (id: number, data: Partial) => {
    try {
      setLoading(true);
      setError(null);
      // API call would go here
