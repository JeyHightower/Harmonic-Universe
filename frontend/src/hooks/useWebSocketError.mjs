...
export function useWebSocketError({
  ...
  const isConnectionError = useCallback((error) => {
    return error.type === 'connection' || error.message?.toLowerCase().includes('connection');
  }, []);

  const isMessageError = useCallback((error) => {
    return error.type === 'message' || error.message?.toLowerCase().includes('message');
  }, []);

  const isCloseError = useCallback((error) => {
    return error.type === 'close' || error.message?.toLowerCase().includes('close');
  }, []);
