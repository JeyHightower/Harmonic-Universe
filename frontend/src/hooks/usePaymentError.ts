...
interface UsePaymentErrorOptions {
  context: string;
  onError?: (error: PaymentError) => void;
  onDeclined?: (error: PaymentError) => void;
  onInsufficientFunds?: (error: PaymentError) => void;
  onInvalidCard?: (error: PaymentError) => void;
  onProcessingError?: (error: PaymentError) => void;
}
...
export function usePaymentError({
  ...
  const isPaymentError = useCallback((error: any): boolean => {
    return (
  ...
  const isCardError = useCallback((error: any): boolean => {
    return (
  ...
  const isRefundError = useCallback((error: any): boolean => {
    return (
