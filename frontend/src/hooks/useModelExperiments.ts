...
export interface ModelExperiments {
  ...
  createExperiment: (data: Partial<Experiment>) => Promise<void>;
  updateExperiment: (id: number, data: Partial<Experiment>) => Promise<void>;
...
export const useModelExperiments = (modelId: number | null): ModelExperiments => {
  ...
  const updateExperiment = useCallback(
    async (id: number, data: Partial<Experiment>) => {
      try {
        setLoading(true);
        setError(null);
        if (!model) return;
