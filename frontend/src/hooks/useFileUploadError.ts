...
interface UseFileUploadErrorOptions {
  context: string;
  onError?: (error: FileUploadError) => void;
  onSizeError?: (file: File) => void;
  onTypeError?: (file: File) => void;
  onNetworkError?: (file: File) => void;
  maxFileSize?: number;
  allowedTypes?: string[];
}
...
export function useFileUploadError({
  ...
  const isFileSizeError = useCallback((error: any): boolean => {
    return (
  ...
  const isFileTypeError = useCallback((error: any): boolean => {
    return (
  ...
  const isUploadError = useCallback((error: any): boolean => {
    return (
