...
export function useMediaError({
  ...
  const isAbortedError = useCallback((error) => {
    return (
      error.code === error.MEDIA_ERR_ABORTED || error.message?.toLowerCase().includes('aborted')
    );
  }, []);

  const isNetworkError = useCallback((error) => {
    return (
      error.code === error.MEDIA_ERR_NETWORK || error.message?.toLowerCase().includes('network')
    );
  }, []);

  const isDecodeError = useCallback((error) => {
    return error.code === error.MEDIA_ERR_DECODE || error.message?.toLowerCase().includes('decode');
  }, []);
