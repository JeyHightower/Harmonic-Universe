  ...
  async (universeId, { rejectWithValue }) => {
    try {
      ...
      try {
        ...
        console.error(
          `[${timestamp}] REDUX-THUNK: Initial getScenes request failed:`,
          initialError
        );
          ...
          try {
            ...
            for (const endpoint of endpoints) {
              ...
              try {
                ...
                if (!fetchResponse.ok) {
                  console.log(
                    `[${timestamp}] REDUX-THUNK: Fetch returned status ${fetchResponse.status}`
                  );
                  continue;
                }
                ...
                console.error(
                  `[${timestamp}] REDUX-THUNK: Fetch error for ${endpoint}:`,
                  fetchError
                );
              }
            }
      ...
      console.log(
        `[${timestamp}] REDUX-THUNK: Successfully fetched ${scenesData.length} scenes using ${endpointUsed}`
      );
