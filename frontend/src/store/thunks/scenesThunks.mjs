...
export const fetchScenes = createAsyncThunk(
  ...
  async (universeId, { rejectWithValue, dispatch, getState }) => {
    ...
    try {
      ...
      console.log(
        `[${timestamp}] REDUX-THUNK: Skipping universe existence check for ${numericUniverseId}, proceeding directly to scene fetch`
      );
      ...
      try {
        ...
        if (initialError.response && initialError.response.status === 404) {
          console.warn(
            `[${timestamp}] REDUX-THUNK: Scenes API returned 404 for universe ${numericUniverseId}, trying fallback`
          );
        }
  ...
  async (sceneData, { rejectWithValue, getState, dispatch }) => {
    try {
      ...
      if (!formattedData.name || formattedData.name.trim() === '') {
        ...
        return rejectWithValue({ message: 'Scene name is required' });
      }
