...
def create_visualization():
    ...
    if data["type"] not in valid_types:
        raise ValidationError(
            f"Invalid visualization type. Must be one of: {', '.join(valid_types)}"
        )
        ...
        try:
            ...
            if str(scene.universe.user_id) != current_user_id:
                raise AuthorizationError(
                    "You don't have permission to add visualizations to this scene"
                )
...
def get_visualizations_by_scene(scene_id):
    ...
    with get_db() as db:
        ...
        if str(scene.universe.user_id) != current_user_id:
            raise AuthorizationError(
                "You don't have permission to view visualizations for this scene"
            )

        # Get visualizations
        visualizations = (
            db.query(Visualization).filter(Visualization.scene_id == scene_id).all()
        )
...
def get_single_visualization(visualization_id):
    ...
    with get_db() as db:
        ...
        if str(visualization.user_id) != current_user_id:
            raise AuthorizationError(
                "You don't have permission to view this visualization"
            )
...
def update_visualization(visualization_id):
    ...
    with get_db() as db:
        ...
        if str(visualization.user_id) != current_user_id:
            raise AuthorizationError(
                "You don't have permission to update this visualization"
            )
...
def delete_visualization(visualization_id):
    ...
    with get_db() as db:
        ...
        if str(visualization.user_id) != current_user_id:
            raise AuthorizationError(
                "You don't have permission to delete this visualization"
            )
...
def generate_visualization(visualization_id):
    ...
    if not is_valid_audio_data(data["audio_data"]):
        raise ValidationError(
            "Invalid audio data format. Must be a valid base64 encoded string."
        )
    ...
    with get_db() as db:
        ...
        if str(visualization.user_id) != current_user_id:
            raise AuthorizationError(
                "You don't have permission to generate this visualization"
            )

        # Here we would process the audio data and generate frames
        # For now, we'll return mock data
        frames = generate_mock_frames(
            visualization.type, data["duration"], data["frame_rate"]
        )
...
def validate_visualization_settings(viz_type, settings):
    ...
    for field in color_fields:
        if settings[field] and not is_valid_color(settings[field]):
            raise ValidationError(
                f"Invalid color format for {field}: {settings[field]}"
            )
    ...
    for i in range(num_frames):
        if viz_type == "particle_system":
            ...
            frame = {
                "frame_number": i,
                "timestamp": i / frame_rate,
                "points": [{"x": j, "y": 50 + 30 * (j % 20) / 20} for j in range(100)],
            }
        elif viz_type == "spectrum":
            frame = {
                "frame_number": i,
