...
def test_physics_constraint_creation(client, scene, auth_headers):
    ...
    response = client.post(
        f"/api/scenes/{scene.id}/physics-constraints",
        json={
            "name": "Spring",
...
def test_force_generation(scene):
    ...
    # Create an object
    obj = PhysicsObjectFactory(
        scene=scene, mass=1.0, position={"x": 0, "y": 0}, velocity={"x": 0, "y": 0}
    )
...
def test_constraint_solving(scene):
    ...
    obj1 = PhysicsObjectFactory(scene=scene, position={"x": 0, "y": 0}, is_static=True)
    obj2 = PhysicsObjectFactory(scene=scene, position={"x": 100, "y": 0})
...
def test_physics_serialization(scene):
    ...
    obj1 = PhysicsObjectFactory(scene=scene)
    obj2 = PhysicsObjectFactory(scene=scene)
    constraint = PhysicsConstraintFactory(scene=scene, object_a=obj1, object_b=obj2)
...
def test_physics_interpolation(scene):
    ...
    assert (
        state1["objects"][0]["position"]["x"]
        < obj_state["position"]["x"]
        < state2["objects"][0]["position"]["x"]
    )
