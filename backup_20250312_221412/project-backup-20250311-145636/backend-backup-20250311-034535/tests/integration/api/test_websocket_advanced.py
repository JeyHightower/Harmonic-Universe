...
def create_test_client(app, user):
    ...
    return SocketIOTestClient(
        app, socketio, namespace="/test", query_string=f"token={token}"
    )
...
def test_concurrent_connections(app, multiple_users, test_universe):
    ...
    try:
        ...
        for i, client in enumerate(clients):
            client.emit(
                "parameter_update",
                {"universe_id": test_universe.id, "parameters": {"value": i}},
                namespace="/test",
            )
        ...
        for client in clients:
            received = client.get_received(namespace="/test")
            parameter_updates = [
                msg for msg in received if msg["name"] == "parameter_updated"
            ]
...
def test_message_ordering(app, test_user, test_universe):
    ...
    try:
        ...
        messages = []
        for i in range(10):
            message = {"universe_id": test_universe.id, "parameters": {"sequence": i}}
        ...
        sequences = [
            update["args"][0]["parameters"]["sequence"]
            for update in updates
            if "parameters" in update["args"][0]
        ]
...
def test_error_handling(app, test_user):
    ...
    try:
        ...
        received = client.get_received(namespace="/test")
        assert any(
            msg["name"] == "error" and "Universe not found" in msg["args"][0]["error"]
            for msg in received
        )
        ...
        received = client.get_received(namespace="/test")
        assert any(
            msg["name"] == "error" and "Invalid data format" in msg["args"][0]["error"]
            for msg in received
        )
        ...
        assert any(
            msg["name"] == "error"
            and "Missing required data" in msg["args"][0]["error"]
            for msg in received
        )
...
def test_stress_concurrent_events(app, test_user, test_universe):
    ...
    try:
        ...
        for i in range(event_count):
            client.emit(
                "parameter_update",
                {"universe_id": test_universe.id, "parameters": {"rapid_update": i}},
                namespace="/test",
            )
