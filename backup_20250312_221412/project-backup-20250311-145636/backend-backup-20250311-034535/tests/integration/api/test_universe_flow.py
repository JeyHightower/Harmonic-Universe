...
class TestUniverseFlow:
    def test_complete_universe_flow(
        ...
        collab_login_response = client.post(
            "/api/auth/login", json={"email": collab_email, "password": "password123"}
        )
        ...
        collab_get_response = client.get(
            f"/api/universes/{universe_id}", headers=collab_headers
        )
    ...
    def test_universe_access_control(self, client, auth_headers):
        ...
        other_get_response = client.get(
            f"/api/universes/{universe_id}", headers=other_headers
        )
    ...
    def test_concurrent_updates(self, client, auth_headers, test_user, socketio_client):
        ...
        update_data_2 = {
            "universe_id": universe_id,
            "parameters": {"physics": {"gravity": 6.0}},
        }
