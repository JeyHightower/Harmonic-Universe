...
from backend.app.schemas.physics.physics_parameter import (
    PhysicsParameterCreate,
    PhysicsParameterUpdate,
)


class CRUDPhysicsParameter(
    CRUDBase[PhysicsParameters, PhysicsParameterCreate, PhysicsParameterUpdate]
):
    """CRUD operations for physics parameters."""
    ...
    async def create_with_scene(
        self, db: AsyncSession, *, obj_in: PhysicsParameterCreate
    ) -> PhysicsParameters:
        ...
        db_obj = PhysicsParameters(
            ...
            custom_parameters={
                k: v.dict() for k, v in obj_in.custom_parameters.items()
            },
        )
        db.add(db_obj)
        await db.commit()
    ...
    async def update_by_scene(
        self, db: AsyncSession, *, scene_id: str, obj_in: PhysicsParameterUpdate
    ) -> Optional[PhysicsParameters]:
    ...
    async def validate_parameters(
        ...
        if not params:
            return {
                "valid": False,
                "errors": ["Physics parameters not found for scene"],
            }
    ...
    async def get_effective_parameters(
        ...
        for param_name in [
            ...
            param = getattr(params, param_name)
            if param["enabled"]:
                result[param_name] = {"value": param["value"], "unit": param["unit"]}
