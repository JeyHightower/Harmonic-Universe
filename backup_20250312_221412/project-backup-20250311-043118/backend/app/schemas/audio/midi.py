...
class MIDIEventBase(BaseModel):
    ...
    timestamp: float = Field(
        ..., ge=0, description="Time in seconds from sequence start"
    )
    ...
    control_number: Optional[int] = Field(
        None, ge=0, le=127, description="Control change number"
    )
    control_value: Optional[int] = Field(
        None, ge=0, le=127, description="Control change value"
    )
    duration: Optional[float] = Field(
        None, ge=0, description="Note duration in seconds"
    )
...
class MIDISequenceBase(BaseModel):
    ...
    loop_start: float = Field(
        default=0.0, ge=0, description="Loop start time in seconds"
    )
    loop_end: Optional[float] = Field(None, description="Loop end time in seconds")
    quantization: float = Field(
        default=0.0, ge=0, description="Quantization value in seconds"
    )
    parameters: Dict = Field(default_factory=dict, description="Additional parameters")
