...
// Basic selectors
export const selectUniversesState = state => state.universe;
...
export const selectUniverseActionInProgress = state => state.universe.actionInProgress;
...
export const selectUniversesSortedByName = createSelector(
    [selectAllUniverses],
    (universes) => {
        if (!universes) return [];
...
export const selectUniversesSortedByDate = createSelector(
    [selectAllUniverses],
    (universes) => {
        if (!universes) return [];
...
export const selectPublicUniverses = createSelector(
    [selectAllUniverses],
    (universes) => {
        if (!universes) return [];
...
export const selectPrivateUniverses = createSelector(
    [selectAllUniverses],
    (universes) => {
        if (!universes) return [];
        return universes.filter(universe => !universe.is_public && universe.visibility !== 'public');
...
export const selectUniverseIdsMap = createSelector(
    [selectAllUniverses],
    (universes) => {
        const idMap = {};
        if (universes) {
...
export const selectIsUniverseBeingDeleted = createSelector(
    ...
    (universeState, universeId) => {
        return universeState.deletingUniverseIds &&
            universeState.deletingUniverseIds.includes(universeId);
    }
);
