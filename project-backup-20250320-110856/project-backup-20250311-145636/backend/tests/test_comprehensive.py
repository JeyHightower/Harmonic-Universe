...
class TestAuthentication:
    ...
    def test_profile_management(self, client, auth_headers):
        ...
        response = client.post(
            ...
            json={"bio": "Test bio", "preferences": {"theme": "dark"}},
        )
        assert response.status_code == 201
    ...
    def test_universe_crud(self, client, auth_headers):
        ...
        response = client.post(
            ...
            json={
                ...
                "physics_parameters": {"gravity": 9.81, "time_dilation": 1.0},
            },
        )
        assert response.status_code == 201
        ...
        # Update universe
        response = client.put(
            f"/api/universe/{universe_id}",
            json={
                "name": "Updated Universe",
                "physics_parameters": {"gravity": 8.0, "time_dilation": 2.0},
            },
            headers=auth_headers,
        )
        assert response.status_code == 200
...
class TestCollaboration:
    def test_collaboration_features(
        self, client, auth_headers, test_user, socket_client
    ):
        ...
        response = client.post(
            "/api/universe",
            headers=auth_headers,
            json={"name": "Collab Universe", "description": "A collaborative universe"},
        )
        assert response.status_code == 201
        ...
        # Add collaborator
        response = client.post(
            f"/api/universe/{universe_id}/collaborators",
            headers=auth_headers,
...
class TestIntegration:
    def test_full_workflow(self, client, auth_headers, socket_client):
        ...
        client.post(
            "/api/profile",
            headers=headers,
            json={"bio": "Integration test bio", "preferences": {"theme": "dark"}},
        )
        ...
        socket_client.emit(
            "parameter_update",
            {"universe_id": universe_id, "parameters": {"gravity": 8.81}},
            auth=headers,
        )
        received = socket_client.get_received()
...
class TestUser:
    def test_user_crud(self, client, auth_headers):
        ...
        # Update user
        response = client.put(
            "/api/auth/me", headers=auth_headers, json={"username": "updated_user"}
        )
        assert response.status_code == 200
        user_data = response.get_json()
