...
class WebGLSettings(BaseModel):
    ...
    pixelRatio: float = Field(
        default=1.0, ge=0.5, le=2.0, description="Device pixel ratio"
    )
    shadowMap: bool = Field(default=True, description="Enable shadow mapping")
    maxLights: int = Field(
        default=4, ge=1, le=8, description="Maximum number of lights"
    )
    postProcessing: Dict[str, bool] = Field(
        default_factory=lambda: {"bloom": True, "ssao": False, "fxaa": True},
...
class AudioAnalysisSettings(BaseModel):
    ...
    fftSize: int = Field(
        default=2048, ge=32, le=32768, description="FFT size for analysis"
    )
    smoothingTimeConstant: float = Field(
        default=0.8, ge=0, le=1, description="Smoothing time constant"
    )
    minDecibels: float = Field(
        default=-100, ge=-150, le=-50, description="Minimum decibels"
    )
    maxDecibels: float = Field(
        default=-30, ge=-80, le=0, description="Maximum decibels"
    )
...
class AdvancedEffect(BaseModel):
    ...
    uniforms: Dict[str, Union[float, List[float]]] = Field(
        default_factory=dict, description="GLSL uniforms"
    )
    blendMode: str = Field(default="normal", description="Effect blend mode")
...
class VisualizationPreset(BaseModel):
    ...
    effects: List[AdvancedEffect] = Field(
        default_factory=list, description="Preset effects"
    )
    parameters: Dict[str, Any] = Field(
        default_factory=dict, description="Preset parameters"
    )
...
class VisualizationSettings(BaseModel):
    ...
    type: str = Field(default="waveform", description="Type of visualization")
    preset: Optional[VisualizationPreset] = Field(
        None, description="Active visualization preset"
    )
    webgl: WebGLSettings = Field(
        default_factory=WebGLSettings, description="WebGL settings"
    )
    audioAnalysis: AudioAnalysisSettings = Field(
        default_factory=AudioAnalysisSettings, description="Audio analysis settings"
    )
    effects: List[AdvancedEffect] = Field(
        default_factory=list, description="Active effects"
    )
...
class VisualizationBase(NameDescriptionModel, MetadataModel):
    ...
    title: str = Field(
        ..., min_length=1, max_length=255, description="Visualization title"
    )
    ...
    audio_file_id: Optional[int] = Field(
        None, gt=0, description="Associated audio file ID"
    )
