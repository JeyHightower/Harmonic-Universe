...
class ExportManager:
    ...
    async def _export_video(self):
        ...
        duration = settings.get("duration", self.scene_data.get("timeline", {}).get("duration", 10))
        ...
        process = (
            ffmpeg
            .input('pipe:', format='rawvideo', pix_fmt='rgb24', s=f"{resolution['width']}x{resolution['height']}")
