...
class PhysicsObjectBase(BaseModel):
    ...
    rotation: Vector3D = Field(default_factory=Vector3D, description="Rotation in euler angles")
    angular_velocity: Vector3D = Field(default_factory=Vector3D, description="Angular velocity vector")
    scale: Vector3D = Field(default_factory=lambda: Vector3D(x=1.0, y=1.0, z=1.0), description="Scale in 3D")
    ...
    collision_shape: str = Field(default="box", description="Shape of the collision volume")
    collision_params: Dict = Field(default_factory=dict, description="Additional collision parameters")
    material_properties: MaterialProperties = Field(default_factory=MaterialProperties, description="Material properties")
