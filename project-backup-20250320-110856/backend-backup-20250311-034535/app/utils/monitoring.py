...
class PerformanceMonitor:
    ...
    def start_request_tracking(self, path: str, method: str) -> RequestMetrics:
        ...
        metrics = RequestMetrics(path=path, method=method, start_time=time.time())
        return metrics

    def end_request_tracking(
        self, metrics: RequestMetrics, status_code: int, error: Optional[str] = None
    ) -> None:
    ...
    def get_performance_report(self) -> Dict:
        ...
        request_stats = {
            ...
            "average_duration": sum(r.duration or 0 for r in requests) / len(requests)
            if requests
            else 0,
        ...
        if resources:
            resource_stats = {
                ...
                "memory": {
                    "average": sum(r.memory_percent for r in resources)
                    / len(resources),
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            metrics = monitor.start_request_tracking(
                path=func.__name__, method="FUNCTION"
            )
            ...
            try:
                ...
                monitor.end_request_tracking(
                    metrics, 500, error=f"{type(e).__name__}: {str(e)}"
                )
                raise
