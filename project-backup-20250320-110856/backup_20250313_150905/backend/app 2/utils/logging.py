...
def rotate_logs(log_dir: Union[str, Path], max_size_mb: float = 10.0,
...
def log_to_json(logger: logging.Logger, level: int, message: str,
...
def sanitize_log_data(data: Dict[str, Any], sensitive_fields: Optional[List[str]] = None) -> Dict[str, Any]:
...
def create_audit_log(logger: logging.Logger, action: str, user: str,
...
def setup_error_email_handler(logger: logging.Logger,
                            smtp_host: str,
                            smtp_port: int,
                            from_addr: str,
                            to_addrs: List[str],
