...
def socket_client(app, test_user):
    ...
    client = SocketIOTestClient(app, socketio, query_string=f"token={access_token}")
    return client
...
def test_complete_universe_flow(client, socket_client, auth_headers):
    ...
    create_data = {
        ...
        "parameters": {
            ...
            "music": {"tempo": 120, "key": "C", "scale": "major"},
        },
    }
    ...
    # 4. Start simulation
    socket_client.emit(
        "start_simulation", {"universe_id": universe_id}, namespace="/test"
    )
    time.sleep(0.1)
    received = socket_client.get_received(namespace="/test")
    assert len(received) > 0
    ...
    # 5. Stop simulation
    socket_client.emit(
        "stop_simulation", {"universe_id": universe_id}, namespace="/test"
    )
    time.sleep(0.1)
    received = socket_client.get_received(namespace="/test")
    assert len(received) > 0
