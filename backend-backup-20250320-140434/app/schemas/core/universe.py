...
class PhysicsParams(BaseModel):
    ...
    gravity: float = Field(
        default=9.81, ge=0, le=100, description="Gravitational acceleration in m/s²"
    )
    air_resistance: float = Field(
        default=0.0, ge=0, le=1, description="Air resistance coefficient"
    )
    elasticity: float = Field(
        default=1.0, ge=0, le=1, description="Elasticity coefficient for collisions"
    )
    friction: float = Field(default=0.1, ge=0, le=1, description="Friction coefficient")
...
class HarmonyParams(BaseModel):
    ...
    resonance: float = Field(default=1.0, ge=0, le=10, description="Resonance factor")
    dissonance: float = Field(default=0.0, ge=0, le=1, description="Dissonance factor")
    harmony_scale: float = Field(
        default=1.0, ge=0.1, le=10, description="Scale factor for harmony calculations"
    )
    balance: float = Field(
        default=0.5, ge=0, le=1, description="Balance between harmony and chaos"
    )
...
class UniverseBase(NameDescriptionModel, MetadataModel):
    ...
    is_public: bool = Field(
        default=False, description="Whether the universe is publicly accessible"
    )
    theme: str = Field(default="fantasy", description="The theme of the universe")
    physics_params: PhysicsParams = Field(
        default_factory=PhysicsParams, description="Physics parameters for the universe"
    )
    harmony_params: HarmonyParams = Field(
        default_factory=HarmonyParams, description="Harmony parameters for the universe"
    )
    story_points: List[StoryPoint] = Field(
        default_factory=list, description="Story points in the universe"
    )
