...
class Scene(BaseModel):
    ...
    physics_overrides: Mapped[Dict[str, Any]] = mapped_column(
        JSONB, default=lambda: {"enabled": False, "parameters": {}}
    )
    ...
    visualization_settings: Mapped[Dict[str, Any]] = mapped_column(
        ...
        default=lambda: {
            ...
            "background": {"type": "gradient", "colors": ["#1a2b3c", "#3c2b1a"]},
        },
    )
    ...
    midi_sequences: Mapped[List["MIDISequence"]] = relationship(
        "MIDISequence", back_populates="scene", cascade="all, delete-orphan"
    )
    ...
    def get_effective_physics_params(
        self, universe_params: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def get_effective_harmony_params(
        self, universe_params: Dict[str, Any]
    ) -> Dict[str, Any]:
