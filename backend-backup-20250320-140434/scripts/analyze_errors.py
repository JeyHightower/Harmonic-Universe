...
""",
    ...
    def _generate_async_fixes(self, errors: List[Dict]) -> List[Dict]:
        ...
        if any(
            "Working outside of application context" in e["message"] for e in errors
        ):
            fixes.append(
                {
...
""",
    ...
    def generate_report(self) -> Dict:
        ...
        return {
            ...
            "error_summary": {
                category: len(errors) for category, errors in self.errors.items()
            },
            "errors_by_category": dict(self.errors),
            "suggested_fixes": dict(self.fixes),
            "fix_order": [
                cat.name
                for cat in sorted(self.categories.values(), key=lambda x: x.severity)
            ],
        }
