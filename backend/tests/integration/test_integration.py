...
def test_complete_universe_workflow(client, auth_headers):
    ...
    # Create storyboard
    storyboard_response = client.post(f'/api/universes/{universe_id}/storyboards',
        json={
            'name': 'Test Storyboard',
            'description': 'Integration test storyboard'
        },
        headers=auth_headers
    )
    ...
    # Create scene
    scene_response = client.post(f'/api/storyboards/{storyboard_id}/scenes',
        json={
            'name': 'Test Scene',
    ...
    # Add physics object
    object_response = client.post(f'/api/scenes/{scene_id}/physics-objects',
        json={
            'name': 'Test Object',
            'object_type': 'circle',
            'position': {'x': 0, 'y': 0},
...
def test_user_collaboration_flow(client):
    ...
    universe_response = client.post('/api/universes',
        json={'name': 'Collaborative Universe'},
        headers=owner_headers
    )
    universe_id = universe_response.json['id']

    # Owner invites collaborator
    invite_response = client.post(f'/api/universes/{universe_id}/collaborators',
        json={
    ...
    edit_response = client.put(f'/api/universes/{universe_id}',
    ...
    activity_response = client.get(f'/api/universes/{universe_id}/activity',
        headers=owner_headers
    )
    assert activity_response.status_code == 200
    assert any(
        a['user_id'] == collaborator.id
        for a in activity_response.json['activities']
    )
...
def test_physics_simulation_flow(client, auth_headers):
    ...
    state_response = client.get(f'/api/scenes/{scene.id}/physics/state',
        headers=auth_headers
    )
    ...
    ball_state = next(
        obj for obj in state_response.json['objects']
        if obj['id'] == ball.id
    )
...
def test_media_integration(client, auth_headers):
    ...
    # Add visual effect
    visual_response = client.post(f'/api/scenes/{scene.id}/visual-effects',
        json={
            'name': 'Fade',
            'effect_type': 'fade',
            'parameters': {
    ...
    # Add audio track
    audio_response = client.post(f'/api/scenes/{scene.id}/audio-tracks',
        json={
            'name': 'Background',
...
def test_websocket_integration(authenticated_websocket_client, scene):
    ...
    authenticated_websocket_client.emit('create_object', {
        'scene_id': scene.id,
        'object_data': {
            'type': 'circle',
            'position': {'x': 0, 'y': 0}
    ...
    authenticated_websocket_client.emit('start_simulation', {
        'scene_id': scene.id
    })
...
def test_error_handling_integration(client, auth_headers):
    ...
    scene = SceneFactory()
    physics_response = client.post(f'/api/scenes/{scene.id}/physics-objects',
        json={
            'object_type': 'invalid',
            'position': {'x': 0, 'y': 0}
        },
    ...
    collab_response = client.post(f'/api/universes/{scene.storyboard.universe.id}/collaborators',
        json={
            'user_id': 9999,
            'role': 'invalid'
        },
