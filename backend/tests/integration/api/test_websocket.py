...
def test_parameter_update(socketio_client, test_universe):
    ...
    # Update parameters
    new_params = {"physics": {"gravity": 7.0}}
    socketio_client.emit(
        "parameter_update",
        {"universe_id": test_universe.id, "parameters": new_params},
        namespace="/test",
    )
    time.sleep(0.1)
    received = socketio_client.get_received(namespace="/test")
...
def test_simulation_control(socketio_client, test_universe):
    ...
    # Start simulation
    socketio_client.emit(
        "start_simulation", {"universe_id": test_universe.id}, namespace="/test"
    )
    time.sleep(0.1)
    received = socketio_client.get_received(namespace="/test")
    assert len(received) > 0
...
def test_unauthorized_access(app):
    ...
    client = SocketIOTestClient(app, socketio, namespace="/test")
    try:
...
def test_multiple_clients(app, test_universe):
    ...
    client1 = SocketIOTestClient(
        app, socketio, namespace="/test", query_string=f"token={token1}"
    )
    client2 = SocketIOTestClient(
        app, socketio, namespace="/test", query_string=f"token={token2}"
    )
...
def test_error_handling(app, socketio_client):
    ...
    invalid_update = {"universe_id": 9999, "parameters": {"invalid": "data"}}
...
def test_authentication(app, unauthenticated_socketio_client, test_universe):
    ...
    unauthenticated_socketio_client.emit(
