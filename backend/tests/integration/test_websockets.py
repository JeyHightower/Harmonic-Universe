...
def test_physics_update(authenticated_websocket_client, scene, physics_object):
    ...
    authenticated_websocket_client.emit('physics_update', {
        'scene_id': scene.id,
...
def test_selection_sync(authenticated_websocket_client, scene, physics_object):
    ...
    authenticated_websocket_client.emit('selection_change', {
        'scene_id': scene.id,
        'selection': selection
    })
...
def test_broadcast_filtering(app, scene):
    ...
    client1 = socketio.test_client(
        app,
        headers={'Authorization': f'Bearer {user1.generate_auth_token()}'}
    )
    client2 = socketio.test_client(
        app,
        headers={'Authorization': f'Bearer {user2.generate_auth_token()}'}
    )
