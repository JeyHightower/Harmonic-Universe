...
def test_effect_preview(client, scene, auth_headers):
    ...
    response = client.get(f'/api/visual-effects/{effect.id}/preview', headers=auth_headers)
...
def test_audio_waveform(client, scene, auth_headers):
    ...
    response = client.get(f'/api/audio-tracks/{track.id}/waveform', headers=auth_headers)
    assert response.status_code == 200
...
def test_effect_template(client, auth_headers):
    ...
    response = client.post('/api/effect-templates/apply',
        json={
