...
def test_storyboard_update(client, storyboard, auth_headers):
    ...
    response = client.put(
        ...
        json={
            ...
            "settings": {"layout": "list", "theme": "light"},
        },
        headers=auth_headers,
    )
...
def test_storyboard_scene_ordering(client, storyboard, auth_headers):
    ...
    response = client.put(
        f"/api/storyboards/{storyboard.id}/scene-order",
...
def test_storyboard_duplicate(client, storyboard, auth_headers):
    ...
    response = client.post(
        f"/api/storyboards/{storyboard.id}/duplicate", headers=auth_headers
    )
    assert response.status_code == 201
...
def test_storyboard_export(client, storyboard, auth_headers):
    ...
    response = client.get(
        f"/api/storyboards/{storyboard.id}/export", headers=auth_headers
    )
    assert response.status_code == 200
...
def test_storyboard_import(client, universe, auth_headers):
    ...
    response = client.post(
        f"/api/universes/{universe.id}/storyboards/import",
...
def test_storyboard_search(client, universe, auth_headers):
    ...
    response = client.get(
        f"/api/universes/{universe.id}/storyboards/search?q=test", headers=auth_headers
    )
    assert response.status_code == 200
...
def test_storyboard_statistics(client, storyboard, auth_headers):
    ...
    response = client.get(
        f"/api/storyboards/{storyboard.id}/stats", headers=auth_headers
    )
    assert response.status_code == 200
...
def test_storyboard_preview(client, storyboard, auth_headers):
    ...
    response = client.get(
        f"/api/storyboards/{storyboard.id}/preview", headers=auth_headers
    )
    assert response.status_code == 200
...
def test_storyboard_sharing(client, storyboard, auth_headers):
    ...
    response = client.post(
        f"/api/storyboards/{storyboard.id}/share",
        json=share_settings,
        headers=auth_headers,
    )
    assert response.status_code == 200
