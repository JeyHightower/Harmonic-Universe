...
def test_access_control(client):
    ...
    owner_response = client.get(f"/api/universes/{universe.id}", headers=owner_headers)
    assert owner_response.status_code == 200

    # Other user should not have access
    other_response = client.get(f"/api/universes/{universe.id}", headers=other_headers)
...
def test_sql_injection_prevention(client, auth_headers):
    ...
    for attempt in injection_attempts:
        response = client.get(f"/api/universes/{attempt}", headers=auth_headers)
        assert response.status_code in [
            404,
            400,
        ]  # Should not expose data or execute injection
...
def test_xss_prevention(client, auth_headers):
    ...
    response = client.post(
        "/api/universes",
        json={"name": xss_payload, "description": "Test universe"},
    ...
    get_response = client.get(
        f'/api/universes/{response.json["id"]}', headers=auth_headers
    )
...
def test_csrf_protection(client, auth_headers):
    ...
    response = client.post(
        "/api/universes", json={"name": "Test Universe"}, headers=headers
    )
...
def test_api_security(client, auth_headers):
    ...
    # Test HTTP methods
    endpoints = ["/api/universes", "/api/users", "/api/storyboards"]
