...
def test_scene_update(client, scene, auth_headers):
    ...
    response = client.put(
        ...
        json={
            ...
            "physics_settings": {"gravity": {"x": 0, "y": -5.0}, "enabled": False},
        },
        headers=auth_headers,
    )
...
def test_scene_list(client, storyboard, scene, auth_headers):
    ...
    response = client.get(
        f"/api/storyboards/{storyboard.id}/scenes", headers=auth_headers
    )
    assert response.status_code == 200
...
def test_scene_physics_objects(client, scene, auth_headers):
    ...
    # Get physics objects
    response = client.get(
        f"/api/scenes/{scene.id}/physics-objects", headers=auth_headers
    )
    assert response.status_code == 200
...
def test_scene_visual_effects(client, scene, auth_headers):
    ...
    # Get visual effects
    response = client.get(
        f"/api/scenes/{scene.id}/visual-effects", headers=auth_headers
    )
    assert response.status_code == 200
...
def test_scene_audio_tracks(client, scene, auth_headers):
    ...
    response = client.post(
        ...
        json={
            ...
            "parameters": {"volume": 0.8, "loop": True, "start_time": 0},
        },
        headers=auth_headers,
    )
    assert response.status_code == 201
...
def test_scene_physics_simulation(client, scene, auth_headers):
    ...
    obj1 = PhysicsObjectFactory(
        scene=scene, position={"x": 0, "y": 100}, velocity={"x": 1, "y": 0}
    )
    obj2 = PhysicsObjectFactory(
        scene=scene, position={"x": 100, "y": 100}, is_static=True
    )

    # Start simulation
    response = client.post(
        f"/api/scenes/{scene.id}/physics/start", headers=auth_headers
    )
    assert response.status_code == 200
...
def test_scene_timeline(client, scene, auth_headers):
    ...
    response = client.put(
        f"/api/scenes/{scene.id}/timeline", json=timeline_data, headers=auth_headers
    )
    assert response.status_code == 200
...
def test_scene_import(client, storyboard, auth_headers):
    ...
    response = client.post(
        f"/api/storyboards/{storyboard.id}/scenes/import",
