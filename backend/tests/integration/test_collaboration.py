...
def test_collaborator_permissions(client, universe):
    ...
    editor_headers = {
        'Authorization': f'Bearer {editor.generate_auth_token()}'
    }
    edit_response = client.put(f'/api/universes/{universe.id}',
    ...
    viewer_headers = {
        'Authorization': f'Bearer {viewer.generate_auth_token()}'
    }
    view_response = client.put(f'/api/universes/{universe.id}',
...
def test_activity_tracking(client, universe, auth_headers):
    ...
    client.put(f'/api/universes/{universe.id}',
        json={'name': 'Updated Universe'},
        headers=auth_headers
    )

    # Get activity log
    response = client.get(f'/api/universes/{universe.id}/activity', headers=auth_headers)
...
def test_real_time_collaboration(authenticated_websocket_client, scene, user):
    ...
    authenticated_websocket_client.emit('edit_operation', {
        'scene_id': scene.id,
        'operation': edit,
        'user_id': user.id
    })
...
def test_access_control_inheritance(client, universe, auth_headers):
    ...
    edit_response = client.put(f'/api/scenes/{scene.id}',
...
def test_collaboration_statistics(client, universe, auth_headers):
    ...
    response = client.get(f'/api/universes/{universe.id}/collaboration-stats',
        headers=auth_headers
    )
    assert response.status_code == 200
...
def test_comment_system(client, scene, auth_headers):
    ...
    # Add comment
    response = client.post(f'/api/scenes/{scene.id}/comments',
        json={
    ...
    response = client.post(f'/api/comments/{comment_id}/replies',
