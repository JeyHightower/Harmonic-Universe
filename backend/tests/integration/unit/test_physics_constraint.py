...
def test_constraint_type_validation(session, test_scene):
    ...
    with pytest.raises(ValueError) as excinfo:
        ...
        constraint.validate_constraint_type()
    assert "angle_limits.max must be greater than angle_limits.min" in str(
        excinfo.value
    )
...
def test_cascade_delete(session, test_scene, test_physics_object):
    ...
    assert (
        session.get(PhysicsConstraint, constraint_id) is None
    ), "PhysicsConstraint should be deleted"
