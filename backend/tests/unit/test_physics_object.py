...
from app.models import PhysicsObject, Scene
...
def test_default_values(session, test_scene):
    ...
    assert physics_object.collision_filter == {"category": 1, "mask": 0xFFFFFFFF}
...
def test_physics_properties_validation(session, test_scene):
    ...
    with pytest.raises(ValueError) as excinfo:
        ...
        physics_object.validate_physics_properties()
    assert "Position must be a dictionary with 'x' and 'y' coordinates" in str(excinfo.value)
...
def test_cascade_delete(session, test_scene):
    ...
    # Verify physics object is deleted
    assert session.query(PhysicsObject).filter_by(id=object_id).first() is None
