...
class Scene(Base):
    ...
    physics_overrides = Column(JSONB)
    harmony_overrides = Column(JSONB)
...
class PhysicsParameter(Base):
    ...
    version = Column(Integer, default=1)
    is_active = Column(Boolean, default=True)
    gravity = Column(JSONB)
    air_resistance = Column(JSONB)
    collision_elasticity = Column(JSONB)
    friction = Column(JSONB)
    temperature = Column(JSONB)
    pressure = Column(JSONB)
    fluid_density = Column(JSONB)
    viscosity = Column(JSONB)
    time_step = Column(JSONB)
    substeps = Column(JSONB)
    custom_parameters = Column(JSONB)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
