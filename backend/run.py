...
app = create_app(test_config={"TESTING": True, "DEBUG": True} if is_testing else None)
...
if __name__ == "__main__":
    ...
    if is_testing:
        ...
        if port is None:
            print(
                f"Error: Could not find an available port after {start_port+10}. Please free up a port or specify a different port."
            )
            sys.exit(1)
        if port != start_port:
