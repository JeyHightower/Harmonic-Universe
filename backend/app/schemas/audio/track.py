...
class AudioAutomationBase(BaseModel):
    ...
    parameter_type: AutomationType = Field(
        ..., description="Type of automated parameter"
    )
    target_id: Optional[str] = Field(
        None, description="Target parameter ID for effects"
    )
    points: List[AutomationPoint] = Field(
        default_factory=list, description="Automation points"
    )
...
class AudioTrackBase(BaseModel):
    ...
    effects: List[AudioEffectBase] = Field(
        default_factory=list, description="Audio effects chain"
    )
    parameters: Dict = Field(default_factory=dict, description="Additional parameters")
...
class AudioTrackCreate(AudioTrackBase):
    ...
    midi_sequence_id: Optional[int] = Field(
        None, description="ID of associated MIDI sequence"
    )
