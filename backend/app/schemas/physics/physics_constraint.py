...
class PhysicsConstraintBase(BaseModel):
    ...
    object_b_id: Optional[int] = Field(
        None, description="ID of the second object (optional)"
    )
    ...
    anchor_b: Optional[Vector3D] = Field(
        None, description="Anchor point on object B in local coordinates"
    )
    axis_a: Vector3D = Field(
        default_factory=lambda: Vector3D(x=0.0, y=1.0, z=0.0),
        description="Primary axis for object A",
    )
    axis_b: Optional[Vector3D] = Field(None, description="Primary axis for object B")
    limits: Dict[str, Any] = Field(
        default_factory=dict, description="Constraint limits and ranges"
    )
    spring_properties: Optional[Dict[str, Any]] = Field(
        None, description="Spring properties for spring constraints"
    )
    breaking_force: Optional[float] = Field(
        None, gt=0.0, description="Force required to break the constraint"
    )
    enabled: bool = Field(
        default=True, description="Whether the constraint is currently enabled"
    )
