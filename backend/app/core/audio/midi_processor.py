...
class MIDIProcessor:
    ...
    def get_tempo_changes(self) -> List[Tuple[float, float]]:
        ...
        times, tempos = self.midi_data.get_tempo_changes()
    ...
    def get_time_signature_changes(self) -> List[Tuple[float, Tuple[int, int]]]:
        ...
        changes = self.midi_data.time_signature_changes
        return [(ts.time, (ts.numerator, ts.denominator)) for ts in changes]
