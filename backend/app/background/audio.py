    ...
    try:
        ...
        with get_db() as db:
            audio_file = AudioFile(
                path=file_path, universe_id=universe_id, features=features
            )
            db.add(audio_file)
            db.commit()
...
def generate_audio_track(
    universe_id: int, parameters: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        with get_db() as db:
            ...
            track = AudioTrack(
                universe_id=universe_id, path=output_path, parameters=parameters
            )
            db.add(track)
            db.commit()

            return {"status": "success", "track_id": track.id, "path": output_path}
