    ...
    try:
        ...
        with get_db() as db_session:
            ...
            if custom_params:
                try:
                    ...
                    if "scale_type" in custom_params:
                        harmony_params["scale_type"] = {
                            "value": custom_params["scale_type"]
                        }
                    if "root_note" in custom_params:
                        harmony_params["root_note"] = {
                            "value": custom_params["root_note"]
                        }

                    # Add melody complexity parameter if provided
                    if "melody_complexity" in custom_params:
                        harmony_params["melody_complexity"] = {
                            "value": custom_params["melody_complexity"]
                        }
            ...
            music_data = generate_music_from_params(
                harmony_params=harmony_params, physics_params=physics_params
            )

            # Return the generated music data
            return jsonify({"universe_id": str(universe.id), "music_data": music_data})
        ...
        with get_db() as db_session:
            ...
            if custom_params:
                try:
                    ...
                    custom_params = json.loads(custom_params)
                    logger.info(
                        f"Using custom parameters with AI style {ai_style}: {custom_params}"
                    )
    ...
    try:
        ...
        with get_db() as db:
            ...
            if str(track.user_id) != current_user_id:
                return (
                    jsonify(
                        {"error": "You don't have permission to access this track"}
                    ),
                    403,
                )
    ...
    try:
        ...
        with get_db() as db:
            ...
            if str(track.user_id) != current_user_id:
                return (
                    jsonify(
                        {"error": "You don't have permission to update this track"}
                    ),
                    403,
                )
...
def process_physics_to_audio():
    ...
    try:
        ...
        with get_db() as db:
            ...
            generated_music = generate_music_from_params(
                harmony_params=harmony_parameters, physics_params=physics_parameters
            )
