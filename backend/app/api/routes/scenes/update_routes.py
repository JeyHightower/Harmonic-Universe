...
def update_scene(scene_id):
    try:
        ...
        current_app.logger.info(f"{'Demo' if is_demo_user else 'Regular'} user {user_id} updating scene {scene_id} in universe {scene.universe_id}")
        ...
        try:
            ...
            if universe.user_id != user_id:  # For updates, require full ownership
                current_app.logger.warning(f"Access denied: {'Demo' if is_demo_user else 'Regular'} user {user_id} attempting to update scene {scene_id} in universe owned by {universe.user_id}")
