...
universe_model = api.model('Universe', {
    'id': fields.Integer(readonly=True, description='The universe identifier'),
    'name': fields.String(required=True, description='The universe name'),
    'description': fields.String(required=True, description='The universe description'),
    'created_at': fields.DateTime(readonly=True, description='Creation timestamp'),
    'updated_at': fields.DateTime(readonly=True, description='Last update timestamp'),
    'owner_id': fields.Integer(readonly=True, description='Universe owner ID'),
    'is_public': fields.Boolean(description='Universe visibility status')
})

universe_create_model = api.model('UniverseCreate', {
    'name': fields.String(required=True, description='The universe name'),
    'description': fields.String(required=True, description='The universe description'),
    'is_public': fields.Boolean(default=False, description='Universe visibility status')
})

universe_update_model = api.model('UniverseUpdate', {
    'name': fields.String(description='The universe name'),
    'description': fields.String(description='The universe description'),
    'is_public': fields.Boolean(description='Universe visibility status')
})

@api.route('/')
class UniverseList(Resource):
    @api.doc('list_universes',
             security='Bearer',
             params={
                 'page': {'in': 'query', 'description': 'Page number', 'default': 1},
                 'per_page': {'in': 'query', 'description': 'Items per page', 'default': 10}
             })
...
class UniverseShare(Resource):
    share_model = api.model('Share', {
        'user_id': fields.Integer(required=True, description='User ID to share with'),
        'permission': fields.String(required=True, enum=['read', 'write'], description='Permission level')
    })
