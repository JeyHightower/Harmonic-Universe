...
def create_physics_constraint():
    ...
    try:
        ...
        object1 = db_session.query(PhysicsObject).filter(
            PhysicsObject.id == data['object1_id'],
            PhysicsObject.scene_id == data['scene_id']
        ).first()

        object2 = db_session.query(PhysicsObject).filter(
            PhysicsObject.id == data['object2_id'],
            PhysicsObject.scene_id == data['scene_id']
        ).first()

        if not object1 or not object2:
            return jsonify({'error': 'One or both physics objects not found in this scene'}), 404
...
def get_physics_constraints():
    ...
    try:
        ...
        constraints = db_session.query(PhysicsConstraint).filter(
            PhysicsConstraint.scene_id == scene_id
        ).all()
...
def get_single_physics_constraint(constraint_id):
    ...
    try:
        # Get the constraint
        constraint = db_session.query(PhysicsConstraint).filter(
            PhysicsConstraint.id == constraint_id
        ).first()
