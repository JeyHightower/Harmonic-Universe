...
class PhysicsEngine:
    ...
    def add_particle(self, position: Vector2D, velocity: Vector2D, mass: float = 1.0,
                    radius: float = 1.0) -> Particle:
    ...
    def apply_air_resistance(self, particle: Particle):
        ...
        air_force = particle.velocity.normalize() * (-0.5 *
    ...
    def get_energy(self) -> Dict[str, float]:
        ...
        kinetic_energy = sum(0.5 * p.mass * p.velocity.magnitude()**2
                           for p in self.particles)
        potential_energy = sum(p.mass * self.parameters.gravity * p.position.y
                            for p in self.particles)
        return {
