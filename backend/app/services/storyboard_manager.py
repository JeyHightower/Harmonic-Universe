...
class StoryboardManager:
    ...
    def add_point(self, point: StoryboardPoint) -> TimelineState:
        ...
        with self.lock:
            ...
            index = bisect.bisect_left([s.timestamp for s in self.timeline], state.timestamp)
    ...
    def remove_point(self, timestamp: float) -> Optional[TimelineState]:
        ...
        with self.lock:
            ...
            if index < len(self.timeline) and abs(self.timeline[index].timestamp - timestamp) < 0.001:
                return self.timeline.pop(index)
        return None

    def get_state_at_time(self, timestamp: float) -> Optional[Tuple[TimelineState, TimelineState, float]]:
        ...
        with self.lock:
            ...
            next_index = bisect.bisect_right([s.timestamp for s in self.timeline], timestamp)
    ...
    def update_harmony_engine(self, timestamp: float):
        ...
        physics_params = self.interpolate_params(
            prev_state.physics_params,
            next_state.physics_params,
            progress
        )

        audio_params = self.interpolate_params(
            prev_state.audio_params,
            next_state.audio_params,
            progress
        )
