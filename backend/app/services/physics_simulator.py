...
class PhysicsSimulator:
    ...
    def apply_friction(self, particle: Dict):
        ...
        if np.any(particle['velocity']):
            friction_force = -self.friction * particle['mass'] * np.sign(particle['velocity'])
            particle['forces'] += friction_force

    def handle_collision(self, particle: Dict, bounds: Tuple[float, float, float, float]):
    ...
    def map_to_audio_params(self) -> Dict:
        ...
        return {
            ...
            'amplitude': min(0.8, max(0.1, avg_energy / 100)),  # Map energy to amplitude
            'filter_cutoff': min(0.9, max(0.1, max_velocity / 20)),  # Map max velocity to filter
            'reverb_amount': min(0.9, max(0.1, self.elasticity))  # Map elasticity to reverb
        }
