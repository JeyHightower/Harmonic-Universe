...
@collaborator_bp.route('/api/universes/<int:universe_id>/collaborators', methods=['GET'])
@jwt_required()
def get_collaborators(universe_id):
    ...
    if not universe.is_public and universe.user_id != current_user_id:
        collab = Collaborator.query.filter_by(universe_id=universe_id, user_id=current_user_id).first()
        if not collab:
...
@collaborator_bp.route('/api/universes/<int:universe_id>/collaborators', methods=['POST'])
@jwt_required()
def add_collaborator(universe_id):
    ...
    existing_collab = Collaborator.query.filter_by(universe_id=universe_id, user_id=user_id).first()
    if existing_collab:
...
@collaborator_bp.route('/api/universes/<int:universe_id>/collaborators/<int:user_id>', methods=['PUT'])
@jwt_required()
def update_collaborator(universe_id, user_id):
    ...
    collaborator = Collaborator.query.filter_by(universe_id=universe_id, user_id=user_id).first_or_404()
...
@collaborator_bp.route('/api/universes/<int:universe_id>/collaborators/<int:user_id>', methods=['DELETE'])
@jwt_required()
