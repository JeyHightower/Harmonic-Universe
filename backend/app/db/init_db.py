...
from backend.app.models.organization.organization import (
    Organization,
    Role,
    Workspace,
    Permission,
...
def create_initial_data(db: Session) -> None:
    ...
    try:
        ...
        if result.scalar() == 0:
            ...
            # Create admin user
            admin_user = User(
                email="admin@harmonicuniverse.com", username="admin", is_active=True
            )
            admin_user.password = (
                "admin123"  # This will use the password property setter
            )
            db.add(admin_user)
